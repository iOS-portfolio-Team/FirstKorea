//
//  JoinCategoryTabViewController.swift
//  TabsView
//
//  Created by 최지석 on 2021/03/05.
//

import UIKit

class JoinCategoryTabViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        var _ = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: timer, userInfo: nil, repeats: false)
        print("나 뷰디드 아이디\(userId)")
        
        buttonJoin.layer.masksToBounds = true
        buttonJoin.layer.cornerRadius = 15
        
    }
    @objc func timerUp(){
        print("a")
        buttonSet()
        print("나 타이머 아이디\(userId)")
    }
    ...
    @IBAction func buttonUpdate(_ sender: UIButton) {
        ...
        let Alert = UIAlertController(title: "완료", message: "회원가입이 완료 되었습니다.", preferredStyle: UIAlertController.Style.alert )
    ...
    func categoryTabStatusSelect(_ userId:String) {
        let urlPath = "http://127.0.0.1:8080/IOS/CategoryTabSelect.jsp?userid="+userId
